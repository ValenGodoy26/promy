...
import { normalizeValidationCode } from "../../services/deepLinks";
...
type CommerceRedemptionsRoute =
  NativeStackScreenProps<CommerceStackParamList, "CommerceRedemptions">["route"];

export default function CommerceRedemptionsScreen() {
  const navigation =
    useNavigation<NativeStackNavigationProp<CommerceStackParamList>>();
  const route = useRoute<CommerceRedemptionsRoute>();
  ...
  const lastAutoValidationCodeRef = React.useRef<string | null>(null);
  ...
  useEffect(() => {
    const deepLinkedCode = route.params?.validationCode
      ? normalizeValidationCode(route.params.validationCode)
      : "";

    if (
      !deepLinkedCode ||
      deepLinkedCode === lastAutoValidationCodeRef.current ||
      submitting
    ) {
      return;
    }

    lastAutoValidationCodeRef.current = deepLinkedCode;
    setValidationCode(deepLinkedCode);
    void submitValidation(deepLinkedCode);
  }, [route.params?.validationCode, submitting]);
  ...
  const openScanner = async () => {
    ...
    if (cameraPermission?.canAskAgain !== false) {
      const shouldContinue = await new Promise<boolean>((resolve) => {
        Alert.alert(
          "Activa la camara para validar mas rapido",
          "PROMY usa la camara solo para leer el QR del cliente. Si prefieres, puedes seguir validando con el codigo manual.",
          [
            { text: "Seguir manual", style: "cancel", onPress: () => resolve(false) },
