...
export function resolveApiBaseUrl() {
  ...
  if (!configured) {
    throw new Error(
      "Falta VITE_API_BASE_URL. Configura promy-web/.env para usar el modo local real.",
    );
  }
...
async function request<T>(
  ...
  headers.set("x-promy-client", "web");
...
export async function confirmEmailChangeRequest(token: string) {
  return request<AuthActionResponse>("/users/confirm-email-change", {
    method: "POST",
    body: { token },
  });
}
...
export async function fetchPublicStats() {
  return request<PublicStatsResponse>("/stats/public", {
    method: "GET",
  });
}
...
export async function updateMyCommerceStatus(
  session: AuthSession,
  body: UpdateMyCommerceStatusInput,
) {
  return request<MyCommerceResponse>("/commerce/me/status", {
    method: "PATCH",
    accessToken: session.accessToken,
    body,
  });
...
export async function fetchAdminBetaRequests(
  session: AuthSession,
  filters?: {
  ...
  return request<AdminBetaAccessRequestsResponse>(`/admin/beta-requests${suffix}`, {
    method: "GET",
    accessToken: session.accessToken,
  });
}
